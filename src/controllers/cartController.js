    ...
    try {
        ...
        if (cartId) {
            ...
            for (let i = 0; i < arrayOfItems.length; i++) {
                if (arrayOfItems[i].productId == productId) {
                    ...
                    const FromCart = await cartModel.findOneAndUpdate({ items: { $elemMatch: { productId: arrayOfItems[i].productId } } }, { $set: finalFilter }, { new: true })
                    return res.send({msg:"quantity increaasing", data:FromCart})
                }
            }
            ...
            const cartDataToAddProduct = {
                ...
                totalPrice: totalPrice,
                totalItems: isCartExists.totaItems+1
            }
      ...
      const  isCartAlreadyCreated = await cartModel.findOne({userId:userId})
        ...
        const cartDataToCreate = {
            ...
            totalPrice: isProductExists.price ,
            totalItems:1,
        }
        ...
        for (let i = 0; i < array.length; i++) {
            if (array[i].productId == productId) {
                ...
                if (removeProduct === 0) {
                    ...
                    return res.status(200).send({ status: true, msg: "directly product removed", data: updateProductItem })
                }
                if (removeProduct === 1) {
                    if (array[i].quantity === 1 && removeProduct === 1) {
                        ...
                        return res.status(200).send({ status: true, msg:"product removed with decreasing quantity", data: removeCart })
                    }
                    ...
                    return res.status(200).send({ status: true, msg: "quantity decreased", data: updateCart })
                }
            }
        }
...
const deletecartById=async function(req,res){
    try {  const userId=req.params.userId
        ...
        if(!isValidObjectId(userId)){
          ...
          const cartdelete=await cartModel.findOneAndUpdate({userId:userId},{$set:{items:[],totalPrice:0,totalItems:0,}},{new:true})
            return res.status(200).send({status:true,msg:"sucessfully deleted",data:cartdelete})
